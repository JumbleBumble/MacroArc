...
///
/// Currently a `BTreeMap` is used, but this may change to a different implementation
/// with a similar interface in a future version of schemars.
/// Even when the `preserve_order` feature flag is set we fall back to `BTreeMap`
/// to avoid relying on external map implementations.
