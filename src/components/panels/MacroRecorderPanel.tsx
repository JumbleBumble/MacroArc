...
export const MacroRecorderPanel: React.FC<MacroRecorderPanelProps> = ({
  ...
  const handleToggle = () => (recording ? onStop(captureName) : onStart(captureName));

  return (
    <section className="glass-panel flex h-full flex-col rounded-3xl p-6">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.4em] text-white/50">Recorder</p>
          <h2 className="text-2xl font-semibold text-white">Macro capture</h2>
        </div>
        <Activity size={22} className={recording ? 'text-brand-accent animate-pulse' : 'text-white/50'} />
      </div>

      <div className="mt-6 flex flex-col gap-3">
        <label className="text-xs uppercase tracking-[0.4em] text-white/50">Session label</label>
        <input
          value={captureName}
      ...
      <div className="mt-6 flex flex-col rounded-2xl border border-white/10 bg-white/5 p-4">
        <span className="text-sm uppercase tracking-[0.5em] text-white/50">Status</span>
        <p className="text-lg font-semibold text-white">{statusText}</p>
        <p className="text-sm text-white/60">{recording ? 'Listening globally' : 'Ready to capture input'}</p>
      </div>
      ...
      {hasPendingCapture && (
        <div className="mt-6 rounded-2xl border border-brand-primary/40 bg-brand-primary/10 p-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.4em] text-brand-secondary">Capture ready</p>
              <p className="text-sm text-white/80">
                {pendingCaptureMetrics.count} events · {formatMilliseconds(pendingCaptureMetrics.duration)}
              </p>
            </div>
      ...
      <motion.button
        ...
        className={`mt-6 flex items-center justify-center gap-3 rounded-2xl px-6 py-4 text-lg font-semibold text-white ${
          recording ? 'bg-red-500/80 shadow-[0_0_25px_rgba(244,63,94,0.45)]' : 'bg-brand-primary/70'
        }`}
      >
        {recording ? <PauseCircle size={22} /> : <PlayCircle size={22} />}
          ...
          {recentEvents.slice(0, 6).map((event) => (
            ...
            >
              <div className="flex items-center gap-3">
                <Circle size={10} className="text-brand-secondary" />
                <span className="text-sm text-white/80">{eventBadge(event)}</span>
              </div>
              <span className="text-xs text-white/50">{formatMilliseconds(event.offsetMs)}</span>
            </div>
          ))}
          {!recentEvents.length && (
