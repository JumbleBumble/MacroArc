...
export const MacroLibraryPanel: React.FC<MacroLibraryPanelProps> = ({
  ...
  <section className="glass-panel rounded-3xl p-6">
    <div className="flex items-center justify-between">
      <div>
        <p className="text-xs uppercase tracking-[0.4em] text-white/50">Macro library</p>
        <h2 className="text-2xl font-semibold text-white">Saved macros</h2>
      </div>
      <button
        ...
        return (
          <motion.div
            ...
            className={`rounded-2xl border px-4 py-4 ${
              isActive ? 'border-brand-primary/60 bg-brand-primary/15' : 'border-white/10 bg-white/5'
            }`}
          >
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="text-sm text-white/60">Last run · {formatTimestamp(macro.lastRun)}</p>
              </div>
            ...
            <div className="mt-4">
              <HotkeyField
                ...
                onChange={(value) => onUpdateHotkey(macro.id, value)}
            ...
            <div className="mt-4 rounded-2xl border border-white/10 bg-black/30 p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <p className="text-sm font-semibold text-white">Loop playback</p>
                <label className="flex items-center gap-2 text-xs uppercase tracking-[0.3em] text-white/60">
                  <input
                    ...
                    onChange={(event) =>
                      onUpdateLoopSettings(macro.id, { enabled: event.target.checked })
                    }
                  />
                  Loop
                </label>
              ...
              {macro.loopEnabled && (
                <div className="mt-3 flex flex-wrap items-center gap-3 text-sm text-white/70">
                  ...
                  />
                  <span className="text-xs text-white/50">Pause between runs</span>
                </div>
              )}
            </div>
          </motion.div>
    ...
    {isPlaying && (
      <p className="mt-4 text-xs uppercase tracking-[0.4em] text-brand-secondary">Playing…</p>
    )}

    <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-[0.4em] text-white/50">Queued macros</p>
          <p className="text-sm text-white/70">{queuedMacros.length} ready</p>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            {queueRunning ? 'Queue running…' : 'Play queued macros'}
          </button>
          <button
      ...
      {queueRunning && (
        <p className="mt-2 text-xs text-brand-secondary">
          Queue active — press {queueHotkey ?? 'queue hotkey'} to stop instantly.
        </p>
      )}
      <div className="mt-4 rounded-2xl border border-white/10 bg-black/30 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-sm font-semibold text-white">Loop queue</p>
          <label className="flex items-center gap-2 text-xs uppercase tracking-[0.3em] text-white/60">
            <input
              type="checkbox"
              className="h-4 w-4 rounded border-white/30 bg-transparent text-brand-primary"
              checked={queueLoopEnabled}
              onChange={(event) => onUpdateQueueLoop({ enabled: event.target.checked })}
            />
            Loop
          </label>
        </div>
        {queueLoopEnabled && (
          <div className="mt-3 flex flex-wrap items-center gap-3 text-sm text-white/70">
            ...
            <input
              ...
              onChange={(event) => {
                const value = Number(event.target.value);
                onUpdateQueueLoop({ delayMs: Number.isNaN(value) ? 0 : value });
              }}
              className="w-28 rounded-full border border-white/20 bg-black/50 px-3 py-1 text-sm text-white focus:border-brand-primary focus:outline-none"
            />
            <span className="text-xs text-white/50">Pause before repeating queue</span>
          </div>
        )}
        <div className="mt-4">
          <HotkeyField
      ...
      <div className="mt-4 max-h-48 space-y-2 overflow-y-auto">
        {queuedMacros.length ? (
          queuedMacros.map((macro) => (
            ...
            >
              <span className="text-sm font-semibold text-white">{macro.name}</span>
          ...
          <p className="rounded-xl border border-dashed border-white/10 px-4 py-6 text-center text-sm text-white/50">
            Nothing queued yet. Use “Queue macro” to stack multiple runs.
          </p>
        )}
      </div>
    </div>
